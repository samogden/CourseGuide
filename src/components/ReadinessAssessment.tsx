...
export function ReadinessAssessment({ courseId, onClose }: ReadinessAssessmentProps) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}>
    <section className="assessment-modal" role="dialog" aria-modal="true" aria-labelledby="assessment-title" onMouseDown={event => event.stopPropagation()}>
      ...
      {document && <iframe ref={iframe} className="assessment-frame" title={`${pack.title} questions`} sandbox="allow-scripts" srcDoc={document} style={{ height: `${iframeHeight}px` }} />}
    </section>
  </div>
}
...
function buildAssessmentDocument(title: string, introduction: string, selections: AssessmentQuestionSelection[], questions: AssessmentQuestionContent[]): string {
  const renderedQuestions = questions.map((question, index) => `<article class="assessment-question" data-skill="${escapeAttribute(selections[index]?.skill ?? '')}"><header><strong>Question ${index + 1}</strong><span>${escapeHtml(selections[index]?.skill ?? '')}</span></header>${question.question_html}<button class="recheck-button" type="button">Check this question</button><details><summary>Explanation</summary>${question.explanation_html}</details></article>`).join('')
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><base href="${window.location.origin}/"><script>window.MathJax={tex:{inlineMath:[['\\(','\\)']]}};</script><script defer src="${mathJaxUrl}"></script><style>${assessmentStyles}</style></head><body><main><p class="intro">${escapeHtml(introduction)}</p><form id="assessment-form">${renderedQuestions}<div class="assessment-footer"><button type="submit">Check all questions</button><button type="button" id="reset">Reset answers</button></div><section id="recap" aria-live="polite"></section></form></main><script>${assessmentScript}</script></body></html>`
}
